import moment from "moment";

export default [
  {
    title: "All Day Event very long title",
    allDay: true,
    start: new Date(2015, 3, 0),
    end: new Date(2015, 3, 1)
  },
  {
    title: "Event shorter than step",
    start: new Date(2015, 3, 10, 8, 0, 0),
    end: new Date(2015, 3, 10, 8, 15, 0)
  },

  {
    title: "Battle of Lepanto: 1571",
    start: new Date(2019, 9, 7),
    end: new Date(2019, 9, 7)
  },
  {
    title: "National Gumbo Day",
    start: new Date(2019, 9, 12),
    end: new Date(2019, 9, 12)
  },
  {
    title: "Battle of Hastings: 1066",
    start: new Date(2019, 9, 14),
    end: new Date(2019, 9, 14)
  },
  {
    title: "Surrender of Yorktown: 1781",
    start: new Date(2019, 9, 19),
    end: new Date(2019, 9, 19)
  },
  {
    title: "National American Beer Day",
    start: new Date(2019, 9, 27),
    end: new Date(2019, 9, 27)
  },
  {
    title: "National Chocolate Day",
    start: new Date(2019, 9, 28),
    end: new Date(2019, 9, 28)
  },
  {
    title: "Halloween",
    allDay: true,
    start: new Date(2019, 9, 31),
    end: new Date(2019, 9, 31)
  },
  {
    title: "Daylight Savings",
    allDay: true,
    start: new Date(2019, 10, 3),
    end: new Date(2019, 10, 3)
  },
  {
    title: "National Sandwich Day",
    allDay: true,
    start: new Date(2019, 10, 3),
    end: new Date(2019, 10, 3)
  },
  {
    title: "National Candy Day",
    allDay: true,
    start: new Date(2019, 10, 4),
    end: new Date(2019, 10, 4)
  },
  {
    title: "National Donut Day",
    allDay: true,
    start: new Date(2019, 10, 5),
    end: new Date(2019, 10, 5)
  },
  {
    title: "Marine Corp Birthday",
    allDay: true,
    start: new Date(2019, 10, 10),
    end: new Date(2019, 10, 10)
  },
  {
    title: "Veterens Day",
    allDay: true,
    start: new Date(2019, 10, 11),
    end: new Date(2019, 10, 11)
  },
  {
    title: "National Fast Food Day",
    allDay: true,
    start: new Date(2019, 10, 16),
    end: new Date(2019, 10, 16)
  },
  {
    title: "EM's Birthday",
    allDay: true,
    start: new Date(2019, 10, 23),
    end: new Date(2019, 10, 23)
  },
  {
    title: "Thanksgiving",
    allDay: true,
    start: new Date(2019, 10, 28),
    end: new Date(2019, 10, 28)
  },
  {
    title: "Black Friday",
    allDay: true,
    start: new Date(2019, 10, 29),
    end: new Date(2019, 10, 29)
  },
  {
    title: "All Saints Day",
    allDay: true,
    start: new Date(2019, 10, 1),
    end: new Date(2019, 10, 1)
  },
  {
    title: "National Pie Day",
    allDay: true,
    start: new Date(2019, 11, 1),
    end: new Date(2019, 11, 1)
  },
  {
    title: "Cyber Monday",
    allDay: true,
    start: new Date(2019, 11, 2),
    end: new Date(2019, 11, 2)
  },
  {
    title: "National Cookie Day",
    allDay: true,
    start: new Date(2019, 11, 4),
    end: new Date(2019, 11, 4)
  },
  {
    title: "St. Nicholas Day",
    allDay: true,
    start: new Date(2019, 11, 6),
    end: new Date(2019, 11, 6)
  },
  {
    title: "Pearl Harbor",
    allDay: true,
    start: new Date(2019, 11, 7),
    end: new Date(2019, 11, 7)
  },
  {
    title: "Poinsettia Day",
    allDay: true,
    start: new Date(2019, 11, 12),
    end: new Date(2019, 11, 12)
  },
  {
    title: "Bill of Rights Day",
    allDay: true,
    start: new Date(2019, 11, 15),
    end: new Date(2019, 11, 15)
  },
  {
    title: "Boston Tea Party: 1773",
    allDay: true,
    start: new Date(2019, 11, 16),
    end: new Date(2019, 11, 16)
  },
  {
    title: "Wright Brother's Day",
    allDay: true,
    start: new Date(2019, 11, 17),
    end: new Date(2019, 11, 17)
  },
  {
    title: "Winter Solstice",
    allDay: true,
    start: new Date(2019, 11, 21),
    end: new Date(2019, 11, 21)
  },
  {
    title: "Christmas Eve",
    allDay: true,
    start: new Date(2019, 11, 24),
    end: new Date(2019, 11, 24)
  },
  {
    title: "Christmas Day",
    allDay: true,
    start: new Date(2019, 11, 25),
    end: new Date(2019, 11, 25)
  },
  {
    title: "Boxing Day",
    allDay: true,
    start: new Date(2019, 11, 26),
    end: new Date(2019, 11, 26)
  },
  {
    title: "St. Stephens Day",
    allDay: true,
    start: new Date(2019, 11, 26),
    end: new Date(2019, 11, 26)
  },
  {
    title: "Hanukkah",
    allDay: true,
    start: new Date(2019, 11, 23),
    end: new Date(2019, 11, 30)
  },
  {
    title: "New Years Eve",
    allDay: true,
    start: new Date(2019, 11, 31),
    end: new Date(2019, 11, 31)
  },
  {
    title: "New Years Day",
    allDay: true,
    start: new Date(2019, 12, 1),
    end: new Date(2019, 12, 1)
  }
];
